...
static const float METERS_PER_SEC = 0.45f;
// If using time turn: duration (ms) to spin ~90° at TURN_PWM
static const uint16_t TURN_TIME_90_MS = 700;
static const int TURN_PWM = 120;

// ====== core utils ======
static float nA(float a){while(a>PI)a-=2*PI;while(a<-PI)a+=2*PI;return a;}

static void mL(int p){int s=abs(p);analogWrite(ENA,s);if(p>=0){digitalWrite(IN1,HIGH);digitalWrite(IN2,LOW);}else{digitalWrite(IN1,LOW);digitalWrite(IN2,HIGH);}}
static void mR(int p){int s=abs(p);analogWrite(ENB,s);if(p>=0){digitalWrite(IN3,HIGH);digitalWrite(IN4,LOW);}else{digitalWrite(IN3,LOW);digitalWrite(IN4,HIGH);}}
static void setM(int L,int R){L=constrain(L,-255,255);R=constrain(R,-255,255);mL(L);mR(R);}
static void brake(){analogWrite(ENA,0);analogWrite(ENB,0);}

static bool waitVis(uint32_t t=6000){uint32_t s=millis();while(millis()-s<t){if(Enes100.isVisible())return true;delay(30);}return false;}
...
static void goForward4m_vision(){
  if(!waitVis()){ goForward4m_time(); return; }
  ...
  while(millis()-t0<timeout){
    ...
    if(!Enes100.isVisible()){ // brief loss tolerance
      ...
      while(!Enes100.isVisible() && millis()-lost<800) { setM(BASE_PWM,BASE_PWM); delay(8); }
      if(!Enes100.isVisible()) { brake(); goForward4m_time(); return; }
    }
  }
  brake();
}
