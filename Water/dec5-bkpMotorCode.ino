...
// dir: any number >= 0 is forwards, any number < 0 is backwards
// power: only values fron 0 to 255 are accepted
void setMotorPow(bool side, int dir, int power) {
  ...
  // continuing, this code is assuming the OUT1 pins are connected to the POSITIVE side, and the OUT2 pins are connected to the NEGATIVE side of the motor.
...
// desAngle can range from pi to -pi
...
void loop() {
  ...
  // then do mission code, will fig out later
  delay(10000);
  obstCh(1.2);
